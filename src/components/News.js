      ...
      <div class="mx-auto">
        <div class="gap-x-2 sm:grid-cols-1 grid md:grid-cols-3 lg:grid-cols-4">
          {this.props.sportsnews.map((article) => {
            return (
              <div class="sm:w-8/10 p-4 md:w-8/10 p-4 lg:w-11/12 p-4 border-2 border-green-600 mb-1 rounded overflow-hidden shadow-lg" key={article.title}>
                <header class="text-green-600 bg-gray-600 font-extrabold text-xl mb-2 p-2" as="h5">
                  {article.title}
                </header>
                <main class="grid justify-items-stretch">
                  <p class="text-white-700 text-base">{article.description}</p>

                  {(article.urlToImage !== null && article.urlToImage !== "")? (
                    <img class="mt-1 w-full rounded shadow-lg"
